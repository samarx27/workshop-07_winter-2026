...
# base layer : gg plot
ggplot(data = abalone_clean, # abalone_clean data
       aes(x = mean_ph, # x mean ph 
           y = change_in_area)) +#change in area is y response variabel
...
**Missing value came from tank B1B**  
...
**With each increase in pH, the model predicts an increase of 3.0 +/- 0.76 in change in shell surface area per day (mm^2^ d^-1^)**  

What is the intercept estimate (with standard error?)  

**when pH is 0, the model predicts the change in shell surface area is -18.5, not to self this is not possible, but is something the model generates**
...
**st. erro: (standard error describing precision in model predictions)**
**conf low and conf high (lower and upper bounds of 95% CU around model predictions)**
